...
public class ChatService {
    ...
    private synchronized void setState(int state) {
        ...
        handler.obtainMessage(FragmentCushion.MESSAGE_STATE_CHANGE, state, -1)
                .sendToTarget();
    }
    ...
    public synchronized void connected(BluetoothSocket socket,
        ...
        Message msg = handler.obtainMessage(FragmentCushion.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(FragmentCushion.DEVICE_NAME, device.getName());
        msg.setData(bundle);
    ...
    private void connectionFailed() {
        Message msg = handler.obtainMessage(FragmentCushion.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(FragmentCushion.TOAST, "Unable to connect device");
        msg.setData(bundle);
        handler.sendMessage(msg);
    ...
    private void connectionLost() {
        Message msg = handler.obtainMessage(FragmentCushion.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(FragmentCushion.TOAST, "Device connection was lost");
        msg.setData(bundle);
        handler.sendMessage(msg);
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    Log.d("bytes message",""+bytes);
                    // Send the obtained bytes to the UI Activity
                    handler.obtainMessage(FragmentCushion.MESSAGE_READ, bytes, -1,
                            buffer).sendToTarget();
                } catch (IOException e) {
                    connectionLost();
                    // Start the service over to restart listening mode
        ...
        public void write(byte[] buffer) {
            try {
                ...
                handler.obtainMessage(FragmentCushion.MESSAGE_WRITE, -1, -1,
                        buffer).sendToTarget();
            } catch (IOException e) {
            }
        }
